...
export default function CompaniesPage() {
  ...
  const { data: npsData, isLoading: npsIsLoading, error } = useQuery<NpsRow[]>({
  ...
  return (
    <Box display="grid" gap={2}>
      ...
      <Card>
        <CardContent>
          <DataTable
            ...
            columns={[
              { key: 'name', header: 'Empresa', render: (name: string, row: Company) => (
              <Button onClick={() => router.push(`/companies/${row.id}`)} color="primary" variant="text" sx={{ textTransform: 'none', cursor: 'pointer' }}>
                {name}
              </Button>
              )},
              { key: 'id', header: 'Ações', render: (id: string) => (
                <Button color="error" disabled={isPending} onClick={() => removeCompany(id)}>Excluir</Button>
              ) }
            ]}
            rows={companiesData ?? []}
            getRowId={(r) => r.id}
      ...
      <Typography variant="h4" fontWeight={700}>NPS por Empresa</Typography>
