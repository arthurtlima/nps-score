    ...
    <Box display="grid" gap={2}>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6">
            NPS atual: <NpsBadge nps={nps?.nps ?? null} />
          </Typography>
          <Typography variant="body2">
            Promotores: {nps?.promoters ?? 0} • Neutros: {nps?.neutrals ?? 0} • Detratores:{' '}
            {nps?.detractors ?? 0} • Total: {nps?.total ?? 0}
          </Typography>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent>
          ...
          <ResponseForm
            companyId={companyId}
            onCreated={() => qc.invalidateQueries({ queryKey: ['responses', companyId] })}
          />
        </CardContent>
      </Card>
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: 'comment', header: 'Comentário' },
              {
                key: 'id',
                header: 'Ações',
                render: (id: string) => <button onClick={() => removeResponse(id)}>Remover</button>,
              },
            ]}
            rows={responses ?? []}
          />
