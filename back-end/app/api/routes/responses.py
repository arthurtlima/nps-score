...
def get_company_responses(company_id: str, db: Session = Depends(get_db)):
    ...
    if not company:
        logger.warning("Company not found for responses fetch", company_id=validated_company_id)
        raise HTTPException(status_code=404, detail="Company not found")
    
    responses = response_repo.get_by_company_id(validated_company_id)
    logger.info("Responses fetched successfully", company_id=validated_company_id, count=len(responses))
    return responses
...
def create_response(company_id: str, response: ResponseCreate, db: Session = Depends(get_db)):
    validated_company_id = validate_uuid(company_id, "company ID")
    logger.info("Creating response for company", company_id=validated_company_id, rating=response.rating)
    ...
    company = company_repo.get_by_id(validated_company_id)
    if not company:
        logger.warning("Company not found for response creation", company_id=validated_company_id)
        raise HTTPException(status_code=404, detail="Company not found")
    
    if response.rating < 0 or response.rating > 5:
        logger.error("Invalid rating provided", rating=response.rating, company_id=validated_company_id)
    ...
    logger.info("Response created successfully", response_id=result.id, company_id=validated_company_id)
    return result
...
def delete_response(response_id: str, db: Session = Depends(get_db)):
    ...
    if not db_response:
        logger.warning("Response not found for deletion", response_id=validated_response_id)
